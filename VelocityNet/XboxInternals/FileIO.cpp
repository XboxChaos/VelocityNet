...
{
	fstr = new fstream(path.c_str(), (std::ios::openmode)(fstream::in | fstream::out | fstream::binary | (fstream::trunc * truncate)));
    if (!fstr->is_open())
...
{
	fstr->seekp(pos, (std::ios::seekdir)dir);
}
